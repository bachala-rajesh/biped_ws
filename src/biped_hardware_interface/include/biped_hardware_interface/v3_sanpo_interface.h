/*
This is the Ver-3 SANPO interface header file. 
...
public:
    ...
    static void unpack_response_and_get_motor_states(std::vector<MotorState>& motor_states, std::vector<uint8_t>& response, const std::vector<uint8_t>& motor_id) {
        ...
        while (true) {
            ...
            // extract the frame
            std::vector<uint8_t>sanpo_response_frame(frame_header_it, frame_header_it + SANPO_FRAME_SIZE); // extract the valid SANPO frame from the response
