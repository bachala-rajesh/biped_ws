/*
This is the Ver-1 SANPO interface header file. 
the method unpack_response_and_get_motor_states() is implemented in an inefficient way.
*/
...
public:
    ...
    static void unpack_response_and_get_motor_states(std::vector<MotorState>& motor_states, std::vector<uint8_t>& response, const std::vector<uint8_t>& motor_id) {
        ...
        while (true) {
            ...
            size_t response_length_till_tail = std::distance(response.begin(), tail_it); //SANPO frame is 18 bytes long.. the tail contains 2 bytes, so the start of the frame is 16 bytes before the tail
            ...
            if (response[frame_start] != SANPO_FRAME_HEADER[0] || response[frame_start + 1] != SANPO_FRAME_HEADER[1]) {
                response.resize(frame_start); // remove the last tail from the response and continue searching for the next tail
                continue; 
            }
