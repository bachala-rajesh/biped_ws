/*
This is the Ver-2 SANPO interface header file. 
...
In this method, the first tail is searched from the back of the response buffer, then check for the header.
*/
...
public:
    ...
    static void unpack_response_and_get_motor_states(std::vector<MotorState>& motor_states, std::vector<uint8_t>& response, const std::vector<uint8_t>& motor_id) {
        ...
        ssize_t search_end_offset = response.size();

        bool _successfully_extracted_atleast_one_frame = false; // flag to track if at least one valid frame has been extracted from the response
        size_t preserve_response_offset_it = response.size(); // iterator to track from where the unprocessed response starts, initialized to the end of the response

        while (true) {

            // break the loop if all the motor IDs have been found in the response
            bool all_frames_received = std::all_of(motor_id_frame_received.begin(), motor_id_frame_received.end(), 
            ...
            auto tail_it = std::find_end(response.begin(), response.begin() + search_end_offset, SANPO_FRAME_TAIL.begin(), SANPO_FRAME_TAIL.end());
            if (tail_it == response.begin() + search_end_offset) break;         // no tail frame found, break the loop

            // check if there is enough data before the tail for a full frame or else exit the loop
            size_t response_length_till_tail_it = std::distance(response.begin(), tail_it); //SANPO frame is 18 bytes long.. the tail contains 2 bytes, so the start of the frame is 16 bytes before the tail
            if (response_length_till_tail_it < (SANPO_FRAME_SIZE - 2)) break;  // not enough data for a full frame before the tail, break the loop

            // check for header frame. if header does not match, remove the last tail and continue searching for the next tail
            size_t frame_start = response_length_till_tail_it - (SANPO_FRAME_SIZE - 2);
            if (response[frame_start] != SANPO_FRAME_HEADER[0] || response[frame_start + 1] != SANPO_FRAME_HEADER[1]) {
                search_end_offset = frame_start; // remove the last tail from the response and continue searching for the next tail
                continue; 
            }
            ...
            if (rx_mit_header != MIT_RX_HEADER) {
                search_end_offset = frame_start; // remove the last frame from the response and continue searching for the next frame
                continue;
            }
            ...
            if (map_it != motor_id_frame_received.end() && !map_it->second) {
                ...
                // data received for the first time
                if (!_successfully_extracted_atleast_one_frame) {
                    _successfully_extracted_atleast_one_frame = true;
                    preserve_response_offset_it = std::distance(response.begin(), tail_it) + SANPO_FRAME_TAIL.size(); // set the preserve_response_offset_it to the index right after the tail of the first successfully extracted frame
                }
                
                // remove the processed frame from the response buffer
                search_end_offset = frame_start; // remove the last frame from the response and continue searching for the next frame
                continue;
            }
            else {
        ...
        if (_successfully_extracted_atleast_one_frame) {
            // remove the processed part of the response buffer, keep the unprocessed part for the next iteration when new response is received and appended to the buffer
            response.erase(response.begin(), response.begin() + preserve_response_offset_it);
        }
    }
};
