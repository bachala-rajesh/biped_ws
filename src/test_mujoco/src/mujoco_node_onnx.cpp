...
int main() {
  ...
  const std::string policy_path = pkg_share + "/models/exported/policy.onnx";
  ...
  test_mujoco::Policy policy(policy_path, use_gpu);
  ...
  const int bench_runs  = 500;

  // Warmup (GPU needs a few runs to initialize kernels)
  for (int i = 0; i < warmup_runs; ++i) policy.forward(stacked);

  // Timed runs
  auto t0 = std::chrono::steady_clock::now();
  for (int i = 0; i < bench_runs; ++i) policy.forward(stacked);
  auto t1 = std::chrono::steady_clock::now();

  double total_ms = std::chrono::duration<double, std::milli>(t1 - t0).count();
  double per_call_us = (total_ms / bench_runs) * 1000.0;

  std::cout << "\n=== Inference Benchmark ===\n";
  std::cout << "  " << bench_runs << " calls in " << total_ms << " ms\n";
  std::cout << "  per call: " << per_call_us << " us\n";
  std::cout << "  budget:   " << (cfg.sim_dt * cfg.decimation * 1e6) << " us per policy step\n";
  std::cout << "===========================\n\n";
