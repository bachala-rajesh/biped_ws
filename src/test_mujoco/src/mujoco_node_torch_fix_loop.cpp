...
int main() {
  ...
  // ---- Timing ----
  // Pace each physics step to wall-clock sim_dt (200 Hz real-time).
  // Render at ~60 Hz by calling viewer.render() every render_every steps.
  const double sim_dt = sim.time_step();  // 0.005 s
  const int render_every = static_cast<int>(std::round(1.0 / (60.0 * sim_dt)));  // ~3

  std::cout << "Starting control loop."
            << "  sim_dt=" << sim_dt << " s"
            << "  render_every=" << render_every << " steps"
            << "  policy_every=" << cfg.decimation << " steps\n";

  using Clock = std::chrono::steady_clock;
  auto next_step_wall = Clock::now();

  while (viewer.is_open()) {

    // ---- Policy step (every decimation physics steps = 50 Hz) ----
    ...
    // ---- Apply control (zero-order hold between policy steps) ----
    for (int i = 0; i < num_joints; ++i) {
      sim.data()->ctrl[i] =
    ...
    // ---- Per-step real-time pacing at sim_dt (200 Hz) ----
    next_step_wall += std::chrono::duration_cast<Clock::duration>(
        std::chrono::duration<double>(sim_dt));
    auto now = Clock::now();
    if (next_step_wall > now) {
      std::this_thread::sleep_until(next_step_wall);
    } else {
      next_step_wall = now;  // fell behind (e.g. render blocked); resync
    }
  }
