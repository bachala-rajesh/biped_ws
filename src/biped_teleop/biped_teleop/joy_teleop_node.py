...
class BIPED_SPEED:
    ...
    MAX_LINEAR_SPEED_X = 1.0
    MAX_LINEAR_SPEED_Y = 1.0
    MAX_ANGULAR_SPEED = 1.0

    LINEAR_SPEED_SCALING = 1
    ANGULAR_SPEED_SCALING = 1
...
class TeleopNode(Node):
    ...
    def __init__(self):
        ...
        # Logging
        self.get_logger().info("joy teleop node Ready")
    ...
    def timer_callback(self):
        ...
        lin_speed_y = BIPED_SPEED.LINEAR_SPEED_Y
        ang_speed = BIPED_SPEED.ANGULAR_SPEED

        # speed scale check
        if self.cmd_map[ACTIONS.LINEAR_SPEED_SCALE_CHANGE]["value"] != 0.0:
            BIPED_SPEED.LINEAR_SPEED_SCALING += math.copysign(
                1, self.cmd_map[ACTIONS.LINEAR_SPEED_SCALE_CHANGE]["value"]
            )
        if self.cmd_map[ACTIONS.ANGULAR_SPEED_SCALE_CHANGE]["value"] != 0.0:
            BIPED_SPEED.ANGULAR_SPEED_SCALING += math.copysign(
                1, self.cmd_map[ACTIONS.ANGULAR_SPEED_SCALE_CHANGE]["value"]
            )

        # apply the speed scaling
        lin_speed_x = BIPED_SPEED.LINEAR_SPEED_X * BIPED_SPEED.LINEAR_SPEED_SCALING
        lin_speed_y = BIPED_SPEED.LINEAR_SPEED_Y * BIPED_SPEED.LINEAR_SPEED_SCALING
        ang_speed = BIPED_SPEED.ANGULAR_SPEED * BIPED_SPEED.ANGULAR_SPEED_SCALING

        # apply the speed limits
        lin_speed_x = min(
            max(lin_speed_x, -BIPED_SPEED.MAX_LINEAR_SPEED_X),
            BIPED_SPEED.MAX_LINEAR_SPEED_X,
        )
        lin_speed_y = min(
            max(lin_speed_y, -BIPED_SPEED.MAX_LINEAR_SPEED_Y),
            BIPED_SPEED.MAX_LINEAR_SPEED_Y,
        )
        ang_speed = min(
        ...
        if self.cmd_map[ACTIONS.LINEAR_X]["value"] != 0.0:
            vel_cmd.twist.linear.x = math.copysign(
                lin_speed_x, self.cmd_map[ACTIONS.LINEAR_X]["value"]
            )

        if self.cmd_map[ACTIONS.LINEAR_Y]["value"] != 0.0:
            vel_cmd.twist.linear.y = math.copysign(
                lin_speed_y, self.cmd_map[ACTIONS.LINEAR_Y]["value"]
            )
